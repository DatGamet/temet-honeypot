...
func init() { Register(removeTimeoutUser{}) }

type removeTimeoutUser struct{}

func (removeTimeoutUser) CustomID() string { return "remove_timeout_user" }

func (removeTimeoutUser) Handle(c *connection.Client, ev *events.InteractionCreateEvent) {
	...
	if reportCase.Err() != nil {
		if errors.Is(reportCase.Err(), mongo.ErrNoDocuments) {
			if _, err := ev.Reply(interactions.ReplyOptions{
				Content: "No active case found for you. Maybe you already resolved the case or a mod did it for you?\nPlease contact <@754246997266923571> if you have any questions.",
				Flags:   discord.MessageFlagEphemeral,
			}); err != nil {
				slog.Error("failed to reply to interaction", "err", err)
			}
	...
	if _, err := ev.Reply(interactions.ReplyOptions{
		Content: "Done. Your timeout was removed.",
		Flags:   discord.MessageFlagEphemeral,
	}); err != nil {
		slog.Error("failed to reply to interaction", "err", err)
	}
